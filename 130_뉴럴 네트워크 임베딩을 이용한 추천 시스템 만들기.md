...
우리의 Generator는 `페어(pairs)`로 학습되었고, 한 배치 당 긍정 샘플  수(`n_positive`) 및 한 배치 당 부정 샘플:긍정 샘플 비율(`negative_ratio`)를 사용합니다. generator는 호출할 때마다 긍정 샘플과 부정 샘플의 새 배치를 생성합니다. 긍정적인 예를 얻기 위해, 우리는 무작위로 true 쌍(pair)을 표본으로 추출합니다. 부정적인 예제의 경우, 무작위로 책과 링크를 샘플링하여 이 쌍이 실제 쌍에 있지 않은지 확인한 다음 배치에 추가합니다.
...
def generate_batch(pairs, n_positive = 50, negative_ratio = 1.0):
    ...
    while True:
        ...
        while idx < batch_size:
            ...
            # 긍정 샘플이 아니라는 것(부정 샘플이라는 것)을 체크합니다.
...
(학습 파라미터가 막연하게 느껴진다면, 어떤 의미에서는 그렇지만, [<U>Deep Learning</U>](https://www.deeplearningbook.org)에 설명된 모범 사례를 기반으로 해보세요. 기계학습의 대부분의 측면과 마찬가지로 신경망 훈련도 대부분 경험적인 결과입니다.)
...
현재, 나는 <U>Bully for Brontosaus</U> 라고 불리는 Stephen Jay Gould의 환상적인 수필을 읽고 있습니다다. 다음에는 무엇을 읽어야 하는지 생각한다면?

![추천](./media/130_5.png)
*내가 다음에 읽어야 할 책에 대한 추천*
...
논픽션과 공상과학 소설에 뚜렷한 부분이 있는 몇 개의 분명한 덩어리가 있습니다. 소설 내용의 다양성을 고려할 때, 소설들은 어디에나 있는 것처럼 보입니다.
...
나는 그 나라들이 얼마나 독특한지 조금 놀랐습니다! 😮 명백히 오스트레일리아의 책들은 매우 독특하게 보입니다.
...
*책과 장르에 가장 많이 연결된 10개의 책들이 보입니다*

<br></br>

TSNE에 대해 주목해야 할 한 가지는 원래 공간에서 벡터 간의 거리를 유지하려고 하지만 치수 수가 줄어들기 때문에 원래 공간을 왜곡할 수 있다는 것입니다. 따라서 50차원 매립 공간에서 서로 가까운 책들은 2차원 TSNE 매립에서 가장 가까운 이웃이 아닐 수 있습니다.
...
*tensorflow의 projector를 사용한 Interactive visualization*
...
1. wikilinks 대신 <U>외부 링크</U>를 사용하여 임베딩을 만듭니다. 이것은 위키피디아의 외부에 있는 웹페이지에 대한 것이며 다른 임베딩 임베딩을 만들 수도 있습니다.

2. 임베딩을 사용하여 지도 학습(supervised learning) 모델을 학습하여 장르, 작가 및 국가가 포함된 책의 특성을 예측할 수 있습니다.
...
2. 문제를 반영하는 임베딩을 배우기 위해 지도 학습(supervised learning) 모델을 구성합니다.
