...
6. `classify.py` : 분류기를 테스트하기 위한 스크립트입니다. 다른 곳에(아이폰 딥러닝 앱이나 라즈베리 파이 딥러닝 프로젝트 같은) 모델을 배포하기 전에 항상 로컬에서 테스트를 거치는 것이 좋습니다. 
...
`build` 함수는 `width`, `height`, `depth`, `classes`, 총 네 가지의 입력변수를 필요로 합니다.
...
위의 합성곱 계층은 32개의 필터와 3 x 3크기의 커널을 가지며, 최종 값은 ReLU 활성화 함수를 거치게 됩니다. 

드롭아웃은 현재 계층에서 다음 계층을 연결하는 노드들을 무작위로 분리하는 과정입니다. 
이러한 무작위 분리 프로세스는 계층 내 어떠한 단일 노드도 특정 클래스, 객체, 가장자리 또는 모서리를 예측하지 못하게 하므로
네트워크의 과적합을 예방하는데 도움이 됩니다. 
...
해당 코드는 `matplotlib`를 위한 백엔드를 설정해준 후, `train.py`에 필요한 모듈들을 가져옵니다.
